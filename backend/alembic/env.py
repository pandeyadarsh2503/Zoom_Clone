...
# When alembic runs from the backend/ directory, Python may not include it on
# sys.path automatically.  Adding it explicitly lets `from app.xxx import yyy`
# work inside this file.
ROOT = Path(__file__).resolve().parent.parent
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

# ── Import settings and all ORM models ───────────────────────────────────────
# Models must be imported before target_metadata is referenced so that
# Base.metadata contains every table and autogenerate detects them all.
